...
- **Shop owners**: Share Recipes they've created, discover Recipes from others.
- **The community**: Small woodworking/manufacturing shops that benefit from
  shared knowledge.
- **Nori project maintainers**: Curate and moderate public Recipes.

## What

A public repository of Recipes that Nori users can browse, fork into their own
Space, and contribute back to. Think of it like GitHub for manufacturing
procedures — you don't need to write a Recipe for changing a Dewalt miter saw
...
- Data model: PublicRecipe entities, fork tracking
...
This is the community play for the open source project. Individual Recipes
(specific products) are proprietary, but process Recipes (tool maintenance,
...
**Not shared:**
- Product-specific Recipes (your proprietary dining table design)
- Customer-specific processes
- Anything with proprietary jigs or trade secrets

### Publishing to the Marketplace

1. Owner selects a Recipe and clicks "Publish to Marketplace"
2. They choose which version to publish (must be a published RecipeVersion)
3. They add marketplace metadata:
   ...
   - Description (why this Recipe is useful)
   - Difficulty level (beginner, intermediate, advanced)
   - Equipment required (tools/machines needed)
4. Personal/proprietary information is stripped or flagged for review
5. Recipe is submitted for moderation (or auto-published, depending on
   trust model)

### Forking

When a shop finds a useful marketplace Recipe:
1. Click "Fork to My Space"
2. A copy of the Recipe is created in their Space as a new Recipe
3. The fork tracks its origin (marketplace Recipe ID + version)
...
- **Search**: Full-text search across Recipe names, descriptions, step titles
- **Filter**: By equipment required, difficulty, popularity
- **Sort**: Most forked, most recently updated, highest rated
- **Featured**: Curated collection of high-quality Recipes

### Contribution Back

If a shop improves a forked Recipe:
...
- **Federated** (more aligned with self-hosting ethos): Each Nori instance
  can expose its public Recipes, and instances discover each other. Much harder
...
MarketplaceRecipe
  ...
  - RecipeData: json (serialized RecipeVersion with steps, media references)
  - Version: int (marketplace version, independent of source Recipe versioning)
...
MarketplaceFork
  - ID: uuid
  - MarketplaceRecipeID: uuid
  - SpaceID: uuid (the shop that forked it)
  - RecipeID: uuid (the local Recipe created from the fork)
...
```
GET    /api/marketplace/recipes                     — Browse/search marketplace
GET    /api/marketplace/recipes/:id                 — Get marketplace Recipe detail
POST   /api/marketplace/recipes                     — Publish a Recipe to marketplace
POST   /api/marketplace/recipes/:id/fork            — Fork to my space
POST   /api/marketplace/recipes/:id/suggest         — Submit improvement suggestion
...
- How should media (photos, videos) be handled in marketplace Recipes? Hosted
...
- What's the intellectual property model? Should there be a license on
  marketplace Recipes? (Creative Commons seems natural.)
- Should shops be able to sell premium Recipes on the marketplace? (Not for v1,
  but could be a sustainability model for the project.)
- How do we prevent low-quality Recipes from flooding the marketplace? Curation
