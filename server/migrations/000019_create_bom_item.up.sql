-- Create bom_item table for bill of materials line items on SOP versions
CREATE TABLE bom_item (
    id SERIAL PRIMARY KEY,
    sop_version_id INT NOT NULL REFERENCES sop_version(id) ON DELETE CASCADE,
    material_id UUID REFERENCES material(id) ON DELETE SET NULL,
    name VARCHAR(255) NOT NULL,
    quantity NUMERIC(12,4) NOT NULL,
    unit VARCHAR(50) NOT NULL,
    sop_step_id INT REFERENCES sop_step(id) ON DELETE SET NULL,
    unit_cost NUMERIC(12,4),
    notes TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Index on sop_version_id for version-scoped queries (primary access pattern)
CREATE INDEX idx_bom_item_sop_version_id ON bom_item(sop_version_id);

-- Index on material_id for material usage lookups
CREATE INDEX idx_bom_item_material_id ON bom_item(material_id);

-- Index on sop_step_id for step-scoped material queries
CREATE INDEX idx_bom_item_sop_step_id ON bom_item(sop_step_id);
