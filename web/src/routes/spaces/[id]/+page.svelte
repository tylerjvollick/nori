  ...
  {:else if currentSpace}
    ...
    <div class="flex justify-between items-center mb-8">
      <div class="flex items-center gap-4">
        <div>
          ...
          <p class="text-muted-foreground mt-1">Manage tasks in this space</p>
        </div>
        <button
          onclick={openSpaceSettings}
          class="p-2 text-muted-foreground hover:text-muted-foreground900 dark:hover:text-white hover:bg-accent rounded-lg transition-colors"
    ...
    {:else if !$sopStore.sops || $sopStore.sops.length === 0}
      <div class="text-center py-12">
        <p class="text-muted-foreground mb-4">No SOPs created yet. Create an SOP first to start creating tasks.</p>
        <Button onclick={() => showCreateSOPModal = true}>Create Your First SOP</Button>
      </div>
    {:else}
      <!-- Kanban Board -->
      ...
      {#if $taskStore.tasks.length === 0}
        <div class="text-center py-12 mt-8">
          <p class="text-muted-foreground mb-4">No tasks yet. Create your first task from an SOP!</p>
...
{#if showCreateTaskDialog}
  <Dialog onClose={handleCancelCreateTask}>
    <div class="p-6">
      <h2 class="text-2xl font-bold text-foreground mb-6">Create New Task</h2>
      ...
      <div class="space-y-4">
        ...
        {#if selectedSOPForTask}
          <div class="mt-4 p-4 bg-secondary/50 dark:bg-secondary/800 rounded-lg">
            <p class="text-sm font-medium text-foreground mb-2">SOP Details:</p>
...
{#if showSpaceSettings && currentSpace}
  <Dialog onClose={handleCancelSettings}>
    <div class="p-6">
      <h2 class="text-2xl font-bold text-foreground mb-6">Space Settings</h2>
...
<!-- Create SOP Modal -->
<CreateSOPModal isOpen={showCreateSOPModal} onClose={() => showCreateSOPModal = false} />
